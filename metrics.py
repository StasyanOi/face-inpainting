...
if __name__ == '__main__':
    files = dataset.sort_names(os.listdir("metrics/custom/real/"))
    total_ssim = 0
    total_psnr = 0
    for i in range(len(files)):
        real = cv2.imread("metrics/custom/real/" + files[i])
        generated = cv2.imread("metrics/custom/generated/" + files[i])
