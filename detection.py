...
def getFace(image):
    ...
    if len(faces) > 0:
        for x, y, w, h in faces:
            ...
            x1 = int(np.mean(left_eye[:, 0]))
            y1 = int(np.mean(left_eye[:, 1]))

            x2 = int(np.mean(right_eye[:, 0]))
            y2 = int(np.mean(right_eye[:, 1]))
            ...
            for x_n, y_n, w_n, h_n in faces_new:
                drect = dlib.rectangle(int(x_n), int(y_n), int(x_n + w_n), int(y_n + h_n))
